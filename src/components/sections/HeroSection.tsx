...
export function HeroSection() {
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden py-16">
      ...
      <div
        className="absolute top-1/2 right-[20%] w-16 h-16 rounded-xl bg-primary/20 border border-primary/30 animate-float"
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            Hosting That <span className="text-gradient">Powers</span>
            <br />
            Your Success
          </motion.h1>
          ...
          >
            Lightning-fast cloud hosting with enterprise-grade security. Deploy
            in seconds, scale without limits, and focus on what matters most.
          </motion.p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {serviceCards.map((card, index) => (
              <motion.div
                ...
                className={`${card.bgClass} border rounded-2xl p-6   duration-250 cursor-pointer hover:shadow-sm hover:shadow-primary hover:border-secondary/30 hover:scale-105 transition-all duration-300`}
              >
                ...
                <h3 className={`font-semibold ${card.textClass} mb-2`}>
                  {card.title}
                </h3>
                <p className={`text-sm ${card.textClass} opacity-70 mb-1`}>
                  Starting at
                </p>
                <p
                  className={`text-3xl font-display font-bold ${card.priceClass} mb-4`}
                >
                  {card.price}
                </p>
                <a
                  href="#"
                  className={`inline-flex items-center gap-1 text-sm font-medium ${card.linkClass} hover:gap-2 transition-all`}
                >
